...
def inno5_i2c_initialize(page_handler):
    status_list = []
    ...
    try:
        if page_handler.message_type.currentText() == InnoPro_MessageType.I2C:
            ...
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.WATCHDOG_REG,
                                                process_watchdog_command(commands.WATCHDOG_OFF).asbyte))
            
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.UVA_REG,
                                                process_uva_command(
                                                    params.UV_MIN_V,
                                                    commands.UVA_RESP_AR,
                                                    commands.UVA_TIMER_8MS,
                                                    commands.UVA_ENABLE_TIMER).asbyte))
            
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.OVA_REG,
                                                process_ova_command(
                                                    params.OV_MAX_V,
                                                    commands.OVA_RESP_AR).asbyte))
            
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.CVO_REG,
                                                process_cvo_command(
                                                    commands.CVO_RESP_AR,
                                                    commands.CVO_TIMER_8MS,
                                                    commands.CVO_CV_CC_MODE).asbyte))
            
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.FWD_PEAK_REG,
...
def inno5_i2c_set_nr(page_handler):
    status_list = []
    i2c_controller:InnoProI2CControllerContainer = page_handler.i2c_controller
    usbpd_sink:PISinkController|PDSinkController = page_handler.usbpd_sink
    try:
        if page_handler.message_type.currentText() == InnoPro_MessageType.I2C:
            ...
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.ISSC_REG,
                                                process_issc_command(
                                                    threshold_bits=commands.ISSC_CC_LIMIT_48,
                                                    threshold_freq=commands.ISSC_FREQ_60KHZ,
                                                    response=commands.ISSC_RESP_NR).asbyte))
            
            status_list.append(cmd_fnc(Inno5Pro_I2C_Registers.VBUSSC_REG,
...
def inno5_send_command_write(page_handler,reg:INNO5_WRITE_REG_UI):
    status = -1
    i2c_controller:Inno5ProI2CController = page_handler.i2c_controller
    usbpd_sink:PISinkController|PDSinkController = page_handler.usbpd_sink
    ...
    try:
        if (reg_address is not None) and (data_lsb is not None) and (data_msb is not None):
            ...
            if page_handler.message_type.currentText() == InnoPro_MessageType.I2C:
                ...
                pass
            if reg_address == Inno5Pro_I2C_Registers.LOOP_OPTION_REG:
                status_list = []
                ...
                if (data_lsb == commands.LOOP_OPTION_CV_LOAD_LSB) & (data_msb == commands.LOOP_OPTION_CV_LOAD_MSB):
                    status_list.append(fnc_write(Inno5Pro_I2C_Registers.CV_LOAD_SREG,join_8bits(commands.SREG_CV_LOAD_BPS_OV_DISABLED_MSB,commands.SREG_CV_LOAD_BPS_OV_DISABLED_LSB)))
                status_list.append(fnc_write(reg_address,u16_val))
...
# Readback Registers 
def inno5_send_command_read(page_handler,read_reg:INNO5_READ_REG_UI):
    status = -1
    i2c_controller:Inno5ProI2CController = page_handler.i2c_controller
...
@i2c_access
@send_btn_update
def inno5_run_cmd_fnc(page_handler, cmd_fnc, *args,**kwargs):
    return cmd_fnc(page_handler,*args,**kwargs)

def inno5_queue_cmd_fnc(page_handler, reg:INNO5_WRITE_REG_UI, *args,**kwargs):
    fnc_name = reg.reg_label
    extra_params = [InnoProFamily.Inno5Pro]
    command = None
    value, ui_params = reg.extract_command_parameters()
    try:
        if value is not None:
            extra_params = [InnoProFamily.Inno5Pro,ui_params]
...
def inno5_run_queue_command_write(page_handler,fnc_name,value:str,extra_params:list):
    try:
        ...
        if not (inno_pro_family == InnoProFamily.Inno5Pro):
            return    
        else:
            reg.send.click()
    except Exception as e:
...
def inno5_queue_readback_fnc(page_handler, reg, *args, **kwargs):
    fnc_name = None
    value = None
    extra_params = [InnoProFamily.Inno5Pro]
    if (type(reg) == INNO5_READ_REG_UI):
...
def inno5_run_queue_command_read(page_handler,fnc_name,value,extra_params:list):
    try:
        reg_index = page_handler.cmd_readback_reg_name_list.index(fnc_name)
        reg = page_handler.cmd_readback_reg_list[reg_index]
        if reg.reg_label == INNO5_READ_REG_UI.reg_label:
            reg.set_parameters_from_data(value)
        if not(extra_params[0] == InnoProFamily.Inno5Pro):
            return    
        else:
            reg.send.click()
    except Exception as e:
...
def inno5_create_reg_ui(page_handler,frame,grid,reg_ui:INNO5_WRITE_REG_UI):
    reg = reg_ui(frame,grid)
    cmd_fnc = inno5_send_command_write
    reg.send.clicked.connect(lambda: inno5_run_cmd_fnc(page_handler,cmd_fnc,reg))
    reg.queue.clicked.connect(lambda: inno5_queue_cmd_fnc(page_handler,reg))
    return reg

def inno5_create_readback_reg_ui(page_handler,frame,grid,reg_ui:INNO5_READ_REG_UI):
    reg = reg_ui(frame,grid)
    cmd_fnc =  inno5_send_command_read
    reg.send.clicked.connect(lambda: inno5_run_cmd_fnc(page_handler,cmd_fnc,reg))
    reg.queue.clicked.connect(lambda: inno5_queue_readback_fnc(page_handler,reg))
    return reg