...
class LoadRegulationTest_2Port(BaseTestObject):
    """
    The load regulation test sweeps the load
    """
    
    title = "Load Regulation 2 Port"
    short_title = "LoadReg"
    ...
    def test_loop(self):
        ...
        for self.vin_index, self.vin_freq in enumerate(self.vin_list): 
            ...
            self.test_data_1.vin_set_V = self.vin_freq[0]
            self.test_data_1.ac_freq_Hz = self.vin_freq[1]
            ...
            self.electronic_load_1.set_load(self.vout_V,max(self.iout_list_A),self.eload_type)
            ...
            for iout_index, iout_level in enumerate(self.iout_list_A):
                ...
                if self.tracking_pdo_requests: 
                    ...
                    if iout_A > iout_previous_A:
                        self.usb_pd_request(vout_V=self.vout_V, 
                            iout_A=iout_A) 
                        self.electronic_load_1.set_load(self.vout_V,iout_A,self.eload_type)
                        self.electronic_load_1.turn_on()
                    else:
                        self.electronic_load_1.set_load(self.vout_V,iout_A,self.eload_type)
                        self.electronic_load_1.turn_on()
                ...
                if self.measure_ripple:
                    ...
                    self.test_data_1.output_ripple_mV = output_ripple_V * 1000
                    self.oscilloscope.run()
                    sleep(1)

                if self.power_meter_load_1 and getattr(self.power_meter_load_1, '_current_auto_range_status', False):
                    self.power_meter_load_1.auto_range_enable(False)
                    sleep(1)
                ...
                self.test_data_1.gather_data(coupling=self.coupling,usb_pd=self.usbpd_test)
                # Use eload for now, add option to use power meter later
                self.test_data_2.gather_data_load(integrate=False, force_use_eload_data= True,usb_pd=self.usbpd_test)
                # print("Gathered data")
                ...
                if self.power_meter_load_1:
                    self.power_meter_load_1.auto_range_enable()
    ...
    def setup_equipment(self):
        ...
        self.power_meter_load_1 = self.equipment.power_meter_load_1
        self.power_meter_load_2 = self.equipment.power_meter_load_2
        
        self.electronic_load_1 = self.equipment.electronic_load_1
        ...
        if self.power_meter_load_1 is not None:
            self.power_meter_load_1.integration_settings(
                mode="NORMAL", timer_s=self.soak_time.integration_time)
            self.power_meter_load_1.stop_integration()
            self.power_meter_load_1.reset_integration()

        if self.electronic_load_1 is not None:
            self.electronic_load_1.reset_values()
    ...
    def setup_data_file(self):
        ...
        self.test_data_1 = TestData()
        self.test_data_1.vout_nom_V = self.vout_V
        self.test_data_1.use_eload_data = self.use_eload_data
        self.test_data_1.source_power_meter = self.power_meter_source
        self.test_data_1.load_power_meter = self.power_meter_load_1
        self.test_data_1.electronic_load = self.electronic_load_1

        self.test_data_2 = TestData()
        self.test_data_2.copy_settings(self.test_data_1)
    ...
    def define_data_header(self):
        ...
        header.add_columns(
            fl = "Input",
            sl = [f'V{self.coupling} (rms)', 'Freq (Hz)'])
        header.add_columns(
            fl = "Input Measurement",
            sl = ['Vin (rms)','Iin (mA)','Pin (W)','PF','%THD'])
        header.add_columns(
            fl = "Output 1 Measurement",
            sl = ['Vo1 (V)','Io1 (A)','Po1 (W)'])
        header.add_columns(
            fl = "Output 2 Measurement",
            sl = ['Vo1 (V)','Io1 (A)','Po1 (W)'])
        header.add_columns(
            fl = "Evaluation",
            sl = ['Efficiency','%V Reg 1','%V Reg 2'])

        
        header_list = header.get_header_list()
        
        return header_list

    def process_data_row(self):
        """Create a row of data from the test data"""
        td1 = self.test_data_1
        td2 = self.test_data_2

        pout_total_W = td1.pout_W + td2.pout_W
        td1.eff_pct = (pout_total_W/td1.pin_W)*100

        input_targ    = [td1.vin_set_V, td1.ac_freq_Hz]
        input_meas    = [td1.vin_V,     td1.iin_mA,     td1.pin_W,      td1.PF,     td1.thd_pct]
        output1_meas  = [td1.vout_V,    td1.iout_A,     td1.pout_W] 
        output2_meas  = [td2.vout_V,    td2.iout_A,     td2.pout_W] 
        eval          = [td1.eff_pct,   td1.vreg_pct,   td2.vreg_pct]
        
        if self.measure_ripple:
            ripple = [td1.output_ripple_mV]
        
        data_row = input_targ + input_meas
        data_row += output1_meas + output2_meas 
        data_row += eval
        ...
        # data_row = [
        #     self.test_data_1.vin_set_V, self.test_data_1.ac_freq_Hz, self.test_data_1.vin_V, 
        #     self.test_data_1.iin_mA,self.test_data_1.pin_W, self.test_data_1.PF, self.test_data_1.thd_pct, 
        #     self.test_data_1.vout_V, self.test_data_1.iout_A, self.test_data_1.pout_W, self.test_data_1.vreg_pct,
        #     self.test_data_1.eff_pct,self.test_data_1.vreg_passfail]
        # if self.measure_ripple:
        #     data_row.append(self.test_data_1.output_ripple_mV)
    ...
    def extract_test_condition(test_item_dict:dict)->dict:
        test_object_class = LoadRegulationTest_2Port
