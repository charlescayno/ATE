...
class I2C_LineSenseTest(BaseTestObject):
    """
    The line regulation test sweeps the line input
    """
    title = "I2C LineSense"
    short_title = "LineSense"
    i2c_test = True
    ...
    i2c_ui_definitions.add_lineedit(label='Primary Turns', param_index=3)
    i2c_ui_definitions.add_lineedit(label='Secondary Turns', param_index=4)
    ...
    i2c_ui_definitions.add_cbx(label="Inno-Pro Family", contents=[InnoProFamily.Inno5Pro], param_index=4)
    ...
    def set_i2c_ui_definitions(self):
        ...
        self.i2c_ui_definitions.add_lineedit(label='Primary Turns', param_index=3)
        self.i2c_ui_definitions.add_lineedit(label='Secondary Turns', param_index=4)
        ...
        self.i2c_ui_definitions.add_cbx(label="Inno-Pro Family", contents=[InnoProFamily.Inno5Pro], param_index=4)
    ...
    tc_default = TestConditions(
        ...
        general_options = GeneralOptions(measure_ripple = False, coupling='DC'),
        ...
        for self.iout_index, iout_level in enumerate(self.iout_list_A):
            ...
            for vin_index, self.vin_freq in enumerate(self.vin_list):
                ...
                if self.measure_ripple:
                    ...
                    waveform_filename = f"{self.short_title}_{self.vin_freq[0]:g}V{self.coupling}_{self.vout_V:g}V_{iout_A:g}A.png"
            ...
            export_to_excel(
                ...
                f"{self.data_filename}", self.sheet_name, anchor) 
    ...
    def input_supply_eload_discharge_sequence(self):
        self.equipment.input_supply_eload_discharge_sequence(max(self.iout_list_A)/3,coupling=self.coupling)    
    ...
    def unpack_test_item(self):
        ...
        # self.measure_ripple:bool = False
    ...
    def setup_data_file(self):
        ...
        if self.sheet_name in sheet_list:
            ...
            clear_sheet(self.output_folder_path,
                    f"{self.data_filename}",self.sheet_name)
            self.wb:Workbook = openpyxl.load_workbook(self.data_file_path)
    ...
    def prepare_test_conditions(self):
        ...
        self.i2c_test_conditions.param[2]= math.floor(self.i2c_test_conditions.param[2])
        self.i2c_test_conditions.param[3]= math.floor(self.i2c_test_conditions.param[3])
        self.n_pri = self.i2c_test_conditions.param[2]
        self.n_sec = self.i2c_test_conditions.param[3]
        # CDC setpoint
    ...
    def define_data_header(self):
        ...
        header_list = [
            ...
            'Mode' ,'LS Ton', 'LS Toff', 'Vin (dc)', 'LS Vin(dc)']
    ...
    def process_data_row(self):
        ...
        td = self.test_data
        self.ls_ton, self.ls_toff = self.i2c_controller.read_line_sense_count()

        ls_ton_adjusted = self.ls_ton - 64
        # Line Sense processing
        if self.sr_zvs_enable == 'Yes':
            add_count = self.sr_zvs_on_count * 16 + 27
            mode = f'ZVS_{self.sr_zvs_on_count:.0f}_{self.sr_zvs_del_count:.0f}' 
        else:
            add_count = 0
            mode = f'QR'
        np_ns = self.n_pri / self.n_sec

        self.vin_est = np_ns * td.vout_V * (self.ls_toff + add_count) / ls_ton_adjusted
        if self.coupling == 'AC':
            vin_dc = td.vin_V*math.sqrt(2)
        else:
            vin_dc = td.vin_V


        data_row = [
            self.load_pct, td.vin_set_V, td.ac_freq_Hz, 
            td.vin_V,td.iin_mA,
            # td.pin_W, td.PF, td.thd_pct, 
            td.vout_V, td.iout_A, td.pout_W, 
            # td.vreg_pct, td.eff_pct,td.vreg_passfail, 
            mode, self.ls_ton, self.ls_toff, vin_dc, self.vin_est]
        if self.measure_ripple:
    ...
    def plot_charts(self):
        ...
        vin_list_V = [ x[0] for x in self.vin_list]
        generate_plots_LineSense(
            vout = self.vout_V,
            iout = self.i_max_A,
            num_step = len(self.load_pct_list),
            vin_step = len(self.vin_list),
            max_vin = max(vin_list_V),
            min_vin = min(vin_list_V),
    ...
    def define_output_data_objects(self):
        ...
        # Plottable Objects
        self.line_sense_vs_vin = PlottableObject(
            title="Line Sense vs Vin",
            type=PlotType.LINE,
            x_label="Line Voltage (Vdc)",
            y_label="Estimated Line Voltage (Vdc)",
    ...
    def create_new_plot_series(self):
        ...
        if self.iout_index == 0:
            ...
            self.line_sense_vs_vin.add_plot_series(
                name=f'Actual Vin',
                x_values=[],
                y_values=[],
    ...
    def update_output_data(self):
        ...
        if self.iout_index == 0:
            self.line_sense_vs_vin.append_plot_data(
                plot_index=0,
                x=td.vin_set_V,
                y=td.vin_set_V)
        
        self.line_sense_vs_vin.append_plot_data(
            plot_index=self.iout_index + 1,
            x=td.vin_set_V,
            y=self.vin_est)

        plottables.append(self.line_sense_vs_vin)
    ...
    def extract_test_condition(test_item_dict:dict)->dict:
        test_object_class = I2C_LineSenseTest
