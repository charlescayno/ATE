...
class LightLoad(BaseTestObject):
    """
    The load regulation test sweeps the load
    """
    
    title = "Light Load"
    short_title = "LightLoad"
    ...
    def get_ui_definitions(self, flags:UIChangeFlags = UIChangeFlags()):
        ...
        if flags.usb_pd_device_toggle_checked:
            ...
            temp_ui_def.nominal_vout_visible = True
    ...
    tc_default = TestConditions(
        ...
        load_range= LoadSettings.LOAD_LIGHT_LOAD_W,
    ...
    def test_loop(self):
        ...
        # self.pout_list_W = [20,15,10,5,2,1,0.5,.25]
        self.pout_list_W = self.load_pct_list
        ...
        for self.vin_index, self.vin_freq in enumerate(self.vin_list):  
            ...
            init_pout_W = self.pout_list_W[0]
            init_iout_A = init_pout_W/self.vout_V
            self.electronic_load.set_load(self.vout_V,init_iout_A,self.eload_type)
            self.electronic_load.turn_on()
            sleep(1)
            self.test_data.gather_data_load_minimal()
            # self.seek_pout_p(self.vout_V, init_pout_W)
            self.electronic_load.cp = init_pout_W
            ...
            for pout_index, pout_level in enumerate(self.pout_list_W):
                # print("Entered inner loop")                 
                self.status_report(self.vin_index, pout_index, vin_delays=False)
                # print("Status report sent")
                # Trim the trailing zeros
                
                
                    # print("Set eload")
                # self.seek_pout_p(self.vout_V, pout_level)
                self.electronic_load.cp = pout_level
                self.electronic_load.turn_on()

                
                # Auto range enable to update the power meter range
                if self.power_meter_load:
                    self.power_meter_load.auto_range_enable()
                if self.power_meter_source:
                    self.power_meter_source.current_auto_range_enable()     
                ...
                if self.measure_ripple:
                    ...
                    waveform_filename = (f"{self.short_title}_{self.vin_freq[0]}V{self.coupling}"
                                         f"_{self.vout_V}V_{pout_level:g}W.png")
                ...
                self.test_data.gather_data(coupling=self.coupling, measure_thd=False,usb_pd=self.usbpd_test)
                # print("Gathered data")
    ...
    def seek_pout_p(self, vout, pout):

        td = self.test_data
        set_load = pout/vout
        iout_res_base = set_load * .0025

        if pout < 0.5:
            iout_res_base = 0.1e-3
        for i in range(0,30):
            # td.get_eload_meas()
            # Above Pout but limit with resolution
            pout_eload = td.iout_A * td.vout_V
            error = abs(pout_eload-pout)/pout
            
            if error > 0.3:
                iout_res = iout_res_base * 3 
            else:
                iout_res = iout_res_base
            
            # 1% error target
            # if (td.iout_A - iout) > 0 or error<0.02:
            if pout <= 0.5 and error < 0.01:
                return
            if pout > 0.5 and  error<0.01:
                return
            elif td.pout_W < pout:
    ...
    def process_data_row(self):
        ...
        data_row = [
            ...
            self.test_data.iin_mA,self.test_data.pin_W, self.test_data.PF,  
            self.test_data.vout_V, self.test_data.iout_A, self.test_data.pout_W,
            self.test_data.eff_pct]
        if self.measure_ripple:
