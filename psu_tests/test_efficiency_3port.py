...
class EfficiencyTest_3Port(BaseTestObject):
    """
    The efficiency test sweeps the load at 100%, 75%, 50%, 25%, and 10% the maximum load current for 3 ports
    """

    title = "Efficiency 3 Port"
    ...
    def test_loop(self):
        ...
        for self.vin_index, self.vin_freq in enumerate(self.vin_list): 
            ...
            self.electronic_load_2.set_load(self.vout_V,iout_A,self.eload_type)
            self.electronic_load_2.turn_on()
            self.electronic_load_3.set_load(self.vout_V,iout_A,self.eload_type)
            self.electronic_load_3.turn_on()

            if iout_A == 0:
                self.electronic_load.turn_off()
                self.electronic_load_2.turn_off()
                self.electronic_load_3.turn_off()
            ...
            for iout_index, iout_level in enumerate(self.iout_list_A):                 
                ...
                self.load_pct = self.load_pct_list[iout_index]
                ...
                if (self.power_meter_load._current_auto_range_status):
                    ...
                    self.power_meter_load_3.auto_range_enable(False)
                    sleep(1)
                ...
                self.test_data_2.gather_data(coupling=self.coupling,usb_pd=self.usbpd_test)
                self.test_data_3.gather_data(coupling=self.coupling,usb_pd=self.usbpd_test)
                
                self.output_dataframe.loc[len(self.output_dataframe)]\
                    = self.process_data_row()
                
                # Effciciency table creation
                # For average efficiency    
                if self.load_pct_list[iout_index] == 25:
                    Ave_eff_temp = np.average(self.output_dataframe['Efficiency'].tail(4))
                    if self.vout_V < standard_low_voltage_V:
                        if Pout_W <= 1:
                            DOE6 = 0.517 * Pout_W  +0.087
                            COC5T2 = 0.517 * Pout_W  +0.091 
                        elif (Pout_W > 1) & (Pout_W <= 49):
                            DOE6 = 0.0834 * math.log(Pout_W) - 0.0014*Pout_W + 0.609
                            COC5T2 = 0.0834 * math.log(Pout_W) - 0.0011*Pout_W + 0.609   
                        elif (Pout_W > 49) & (Pout_W <= 250):
                            DOE6 =  0.87
                            COC5T2 = 0.88
                        else:
                            DOE6 = 0.875
                        ...
                        if Pout_W <= 1:
                            DOE6 = 0.5 * Pout_W  +0.16
                            COC5T2 = 0.5 * Pout_W  +0.169
                        elif (Pout_W > 1) & (Pout_W <= 49):
                            DOE6 = 0.071 * math.log(Pout_W) - 0.0014*Pout_W + 0.67
                            COC5T2 = 0.071 * math.log(Pout_W) - 0.00115*Pout_W + 0.67
                        elif (Pout_W > 49) & (Pout_W <= 250):
                            DOE6 =  0.88
                            COC5T2 = 0.89
                        else:
                            DOE6 = 0.875
                            COC5T2 = 0.89
                    DOE6 = round(DOE6*100,2)
                    COC5T2 = COC5T2*100
                    if Ave_eff_temp >= COC5T2:
                        Eff_crit = "PASS"
                    else:
                        Eff_crit = "FAIL" 
                        
                    # Write average efficiency on table
                    self.efficiency_dataframe.loc[len(self.efficiency_dataframe)] \
                        = [Ave_eff_temp,DOE6,COC5T2,None,f"{Eff_crit} with {round(Ave_eff_temp-COC5T2,2)}% margin"]
                
                # For 10% efficiency
                elif  self.load_pct_list[iout_index] == 10:
                    eff_temp_10 = self.test_data.pout_W/self.test_data.pin_W*100
                    if self.vout_V < standard_low_voltage_V:
                        if (Pout_W > 0.3) & (Pout_W <= 1):
                            COC5T2_10 = 0.517*Pout_W
                        elif (Pout_W > 1) & (Pout_W <= 49):
                            COC5T2_10 = 0.0834 * math.log(Pout_W) \
                                - 0.00127 *Pout_W + 0.518        
                        elif (Pout_W > 49) & (Pout_W <= 250):
                            COC5T2_10 = 0.78
                        else:
                            COC5T2_10 = 0.78             
                        ...
                        if (Pout_W > 0.3) & (Pout_W <= 1):
                            COC5T2_10 = 0.5*Pout_W+0.06
                        elif (Pout_W > 1) & (Pout_W <= 49):
                            COC5T2_10 = 0.071 * math.log(Pout_W) \
                                - 0.00115*Pout_W + 0.570       
                        elif (Pout_W > 49) & (Pout_W <= 250):
                            COC5T2_10 = 0.79
                        else:
                            COC5T2_10 = 0.79  
                    COC5T2_10 = COC5T2_10*100
                    if eff_temp_10 >= COC5T2_10:
                        Eff_crit = "PASS"
                    else:
                        Eff_crit = "FAIL" 
                    
                    # Write 10% efficiency on table    
                    self.efficiency_dataframe.loc[len(self.efficiency_dataframe)] \
                        = [None,None,None,COC5T2_10,f"{Eff_crit} with {round(eff_temp_10-COC5T2_10,2)}% margin"]   
                
                else:
                    # Default input on efficiency table
                    self.efficiency_dataframe.loc[len(self.efficiency_dataframe)] = [None,None,None,None,None] 
            ...
            # Get anchor
            if self.vin_index > 0:
                anchor = f"A{5+1+self.vin_index*(len(self.iout_list_A)+1)}"
                eff_anchor = f"N{5+1+self.vin_index*(len(self.iout_list_A)+1)}"
            else:
                anchor = "A5"
                eff_anchor = "N5"
    ...
    def setup_equipment(self):
        ...
        self.power_meter_load_3 = self.equipment.power_meter_load_3
        self.electronic_load = self.equipment.electronic_load_1
        self.electronic_load_2 = self.equipment.electronic_load_2
        self.electronic_load_3 = self.equipment.electronic_load_3
        ...
        self.electronic_load_3.reset_values()
    ...
    def setup_data_file(self):
        ...
        self.test_data_2.vout_nom_V = self.vout_V
        ...
        self.test_data_3 = TestData()
        self.test_data_3.vout_nom_V = self.vout_V
        self.test_data_3.use_eload_data = self.use_eload_data
        self.test_data_3.load_power_meter = self.power_meter_load_3
        self.test_data_3.electronic_load = self.electronic_load_3
    ...
    def define_data_header(self):
        ...
        header_list = [
            'Load',f'V{self.coupling} (rms)','Freq (Hz)','Vin (rms)','Iin (mA)',
    ...
    def process_data_row(self):
        ...
        # Calculate total power and overall efficiency
        pout_total_W = self.test_data.pout_W + self.test_data_2.pout_W + self.test_data_3.pout_W
        if self.test_data.pin_W > 0:
            eff_pct = (pout_total_W / self.test_data.pin_W) * 100
        else:
            eff_pct = 0
            
        # Store for the table
        self.test_data.eff_pct = eff_pct
        
        data_row = [
            self.load_pct,self.test_data.vin_set_V, self.test_data.ac_freq_Hz, self.test_data.vin_V, 
            self.test_data.iin_mA,self.test_data.pin_W, self.test_data.PF, self.test_data.thd_pct, 
            self.test_data.vout_V, self.test_data.iout_A, self.test_data.pout_W, 
            self.test_data_2.vout_V, self.test_data_2.iout_A, self.test_data_2.pout_W,
            self.test_data_3.vout_V, self.test_data_3.iout_A, self.test_data_3.pout_W,
            eff_pct]
